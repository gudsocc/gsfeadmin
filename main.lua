--[[
    Created by gudsocc @ GitHub.
    DISCORD: its just a prank bro#2229
--]]
...
function CreateSave()
	writefile("oof.txt", defaultSettings)
	wait(0.5)
	local content = readfile("oof.txt")
...
function fullUpdate()
	...
	wait(0.2)
	writefile("oof.txt", fullUPDATED)
end
if writefile ~= nil then
	function builder()
		local TESTsave = readfile("oof.txt")
...
CMDBARText.Changed:Connect(function()
	if CMDBARText.Text ~= "" and CMDBARText.Text ~= commandPrefix then
		...
		for i,cmdtext in pairs(CMDS) do
			if string.find(cmdtext, CMDBARText.Text) then
...
Commands.credits = function(args)
	Notification("info", "GS FE Admin was made by its just a prank bro#2229 , "..commandPrefix.."fullcredits for all credits.", 8)
end
...
Commands.fullcredits = function(args)
	...
	Notification("info", "Credit to DEX creators ("..commandPrefix.."explorer) and xFunnieuss", 1)
end
...
Commands.version = function(args)
	Notification("info", "Current GS FE Admin Version: V2.5", 7)
end
